...
export async function Tab1(){
    ...
    function create_content(data) {
        data.forEach((_d) => {
            _d.content = <orui-radio data={[{name:_d.name,display_name:_d.task_name}]} onClick={(ev) => {
                remove_item(_d)
            }
            }/>
        })
    ...
    if(!window.localStorage.getItem('todo_data')){
        const mock_data = await mock_get_data(create_todo(20)) as []
    ...
    const html = (
        <>
            ...
            <orui-popup ui_name='add_new_todo_pop' show_operation_bar="true" body_style='height: 80%'>
                <orui-button text="取消" type="none" color="primary" slot="operation_bar_left" onClick={() => {pop.visible = false;}}/>
                <orui-button text="确认" type="none" color="primary"  slot="operation_bar_right" onClick={add_item}/>
                <orui-grid>
                    <orui-row grow="0">
                        <orui-form
                            ui_name="add_item_form"
                            layout="horizontal">
                            <orui-form-item
                                name="task_name"
                                display_name="任务名"
            ...
            <orui-date-picker
                ui_name="datePicker"
                onConfirm={date_picker_confirm}
                precision='second'
                min='2024'
                onCancel={() => {datePicker.visible = false}}/>
        </>
    )
