...
export class Input extends ORUIElement {
    ...
    show_count:string | boolean;
    password_icon_container:HTMLElement;
    max_length:string;
    limit_box:HTMLElement;
    clearable:string | boolean;
    clear_icon_path:string; //
    clear:ORUIElement;
    customize_icon_path:string; //自定义标签
    customize_icon:ORUIElement;
    password_icon_visible_path:string;
    password_icon_invisible_path:string;
    input_container:HTMLElement;
    is_textarea:string | boolean;
    rows:string | number;

    disabled:boolean; //禁用 0/1
    readonly:boolean; //只读 0/1
    static INPUT_RIGHT_PADDING = {
    ...
    create(){
        const input_container = (
            <div class='orui-input-container'/>
        )
        this.import_css('/components/input/index.css')
        const input_value = (<div class='orui-input-value'/>)
        const input_value_inner = (<div class='orui-input-value-inner'/>)
        const input_box = (<div class='orui-input-box'/>)
        if(!this.is_textarea){
            const input = (<input class='orui-input'/>)
            this.input = input
            input_box.appendChild(input)
        }else {
            const input = (<textarea class='orui-teatarea'/>)
            if(this.rows){
        ...
        if(this.type == 'password'){
            ...
            const password_visible_icon = (<orui-icon path={this.password_icon_visible_path} style='display:none'/>)
    ...
    create_icon(container,path){
        const icon_container = (
            <div class='orui-icon-container'>
                <orui-icon class="orui-input-icon" path={path}></orui-icon>
            </div>
        )
