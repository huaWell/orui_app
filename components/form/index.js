...
export class Form extends ORUIElement {
    get items() {
        const form_items = this.querySelectorAll('orui-form-item');
        return form_items;
    }
    get_item(name) {
    ...
    init_item_events() {
        const form_items = this.items;
        form_items.forEach(form_item => {
            form_item.addEventListener('change', (e) => {
    ...
    create_form_item(info) {
        ...
        const form_item = ORUI.createElement("orui-form-item", { name: name, display_name: display_name, is_readonly: is_readonly, is_visible: is_visible, is_nullable: is_nullable, editor: editor, format: _format, layout: this.layout });
        return form_item;
    }
    async validate() {
        let verified = true;
