...
export function PulltoRefreshDemo() {
    const html = (ORUI.createElement("orui-title", { text: "\u57FA\u672C\u7528\u6CD5" },
        ORUI.createElement("orui-pull-to-refresh", { onRefresh: () => {
                const message = get_control('message', html);
                const list = get_control('list', message);
                list.refresh();
            } },
            ORUI.createElement(Message, { ui_name: "message" }))));
    return html;
}
