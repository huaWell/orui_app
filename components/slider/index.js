...
export class Slider extends ORUIElement {
    ...
    create() {
        const container = ORUI.createElement("div", { class: 'slider-container' });
        this.container = container;
        this.sliderBar = ORUI.createElement("div", { class: 'slider-bar' });
        const fill = ORUI.createElement("div", { class: 'orui-slider-fill' });
        this.fill_ctr = fill;
        this.sliderThumb = ORUI.createElement("div", { class: 'slider-thumb' });
        const slider_icon = ORUI.createElement("orui-icon", { path: this.icon, color: this.color });
        this.sliderThumb['icon'] = slider_icon;
        this.sliderThumb.appendChild(slider_icon);
        this.sliderBar.appendChild(fill);
        if (this.ticks) {
            const ticks_container = ORUI.createElement("div", { class: 'orui-slider-ticks' });
        ...
        if (this.marks) {
            const marks_container = ORUI.createElement("div", { class: 'orui-slider-marks' });
    ...
    create_ticks() {
        ...
        for (let i = 0; i <= copies; i++) {
            const span = ORUI.createElement("span", { class: 'orui-slider-tick' });
    ...
    create_marks(marks_container) {
        const copies = this.get_total_copies();
        for (let i = 0; i <= copies; i++) {
            const span = ORUI.createElement("span", { class: 'orui-slider-mark' });
            span.style.left = i / copies * 100 + '%';
            span['value'] = multiToFixed(i, this.step, copies);
