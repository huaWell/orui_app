//深拷贝
export const deep_copy = (obj: unknown) => {
...
export const multiToFixed = (num1, num2, total = 10) : number =>{
    return (num1 * total) * (num2 * total) / (total * total)
};

//通过ui_name获取组件
export const get_control = (ui_name: string, dom: any = document) => {
    const control = dom.querySelector(`[ui_name="${ui_name}"]`);
    return control;
};

//创造 通过ui_name获取组件 的方法 根元素为传入的dom
export const use_get_control = (dom: any) => {
    return (ui_name: string) => {
...
export const addDecimalsWithPrecision = (num1, num2, digits) => {
    // 将两个数字字符串补全到相同的长度
    function padNumbers(num1: string, num2: string,digits): [string, string] {
        ...
        // Pad fractional part
        while (frac1.length < frac2.length || frac1.length < digits) frac1 += '0';
        while (frac2.length < frac1.length || frac2.length < digits) frac2 += '0';

        return [`${int1}.${frac1}`, `${int2}.${frac2}`];
    }

    // 从字符串中删除前导零并当超出精度时截取到相应精度
    function trimLeadingZeros(strNum: string,digits): string {
    ...
    // 检查字符串数字是否为负
    function isNegative(strNum: string): boolean {
    ...
    // 减法
    function subtractStrings(num1: string, num2: string,digits): string {
        // 如果有负号，则移除负号并调用加法
    ...
    // 比较两个字符串表示的数字的绝对值大小
    function compareAbsolute(num1: string, num2: string,digits): number {
        // 补全数字字符串
        let [paddedNum1, paddedNum2] = padNumbers(num1, num2,digits);
...
export const formatterTime = (date,format) => {
    ...
    const replacements = {
        ...
        'MM': pad(date.getMonth() + 1), // 月份是从0开始的
        'DD': pad(date.getDate()),
 ...
 * @param time 单位ms
 */
export const mock_get_data = (mock_data: any, time = 3000) => {
