...
export class Tabs extends ORUIElement {
    ...
    custom_style: string;
    fragment:any;
    underline_width: string;
    _active_underline: HTMLElement;
    wraps: any = [];
    ...
    create(){
        ...
        const template = (
            <>
                <div class='orui-tabs'>
                    <div class='orui-tabs-header'>
                        <div class='orui-tabs-list'>
                            <div class='orui-tabs-line' style={`width:${this.underline_width}`}></div>
                            {
                                data.map((_item) => {
                                    const item = (<div class='orui-tabs-wrapper' key={_item.key} onClick={this.tab_item_click.bind(this)}>
                                        <div class='orui-tabs-tab'>{_item.title}</div>
                                    </div>)
                                    return item
                                })
                            }
                        </div>
                        <div class='orui-tabs-content'>
                            <slot></slot>
                        </div>
                    </div>
                </div>
    ...
    swap_content(key){
        this.items.forEach((_item) => {
            (_item as HTMLElement).style.direction = this.direction == 'left'?'ltr':'rtl'
            if(_item['key'] == key && (_item as HTMLElement).style.display != 'block'){
                (_item as HTMLElement).style.display = 'block'
            }else {
                (_item as HTMLElement).style.display = 'none'
            }
        })
    }
    ...
    traverse_children(){
        ...
        for(let i = 0; i < tabItems.length; i++){
            ...
            if(tabItems[i]['key'] != this.active_key){
                (tabItems[i] as HTMLElement).style.display = 'none'
            }
            tabItems[i]['data'] = obj
            _data.push(obj)
        }
